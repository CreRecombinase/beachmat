...
Otherwise, they are automatically realized via the `realize` method in the `r Biocpkg("DelayedArray")` package.
The same realization backend is the same as that specified in R, and can be determined by calling `getRealizationBackend()`.
If the realized matrix is to be reused later, it may be more efficient to perform the realization once in R and pass the result to `.Call`.
